...
func (r *Product) SoftDelete(ctx context.Context, id string) error {
	...
	// Not found = either no such id, or already deleted. We
	// treat "already deleted" as success; surface real not-found
	// via a follow-up existence check.
	if res.RowsAffected == 1 {
		return nil
	}
	// Distinguish "not found" from "already deleted".
	var row ProductModel
	...
	// Row exists with deleted_at set: idempotent success.
	return nil
}
